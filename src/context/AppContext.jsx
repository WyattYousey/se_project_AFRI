...
export const AppProvider = ({ children }) => {
    ...
    const [status, setStatus] = useState(
        USE_MOCK_ANALYSIS ? 'complete' : 'idle'
    ...
    // WARNING: Client-side keys are INSECURE for production
    const clientApiKey = import.meta.env.VITE_GEMINI_API_KEY;
    const isDev = import.meta.env.DEV === true;

    // Warn about API key configuration on mount (useEffect to avoid repeated warnings)
    useEffect(() => {
        if (clientApiKey) {
            if (!isDev) {
                console.error(
                    '🔴 SECURITY ISSUE: VITE_GEMINI_API_KEY is exposed in production build. This key is visible in browser and source maps. Follow these steps:\n' +
                        '1. Rotate the key immediately in Google AI Studio\n' +
                        '2. Remove VITE_GEMINI_API_KEY from .env.local\n' +
                        '3. Set GENAI_API_KEY via Netlify environment variables\n' +
                        '4. Use server-side /api/analyze endpoint only\n' +
                        'See: https://ai.google.dev/gemini-api/docs/api-key'
                );
            } else {
                console.warn(
                    '⚠️  Development mode: Using VITE_GEMINI_API_KEY for direct client-side calls. This is DEV ONLY and insecure for production.'
                );
            }
        }
    }, [clientApiKey, isDev]);
        ...
        new Promise((resolve) => setTimeout(resolve, 800))
            .then(() => {
                ...
                function parseDataUrl(dataUrl) {
                    const m =
                        dataUrl && dataUrl.match(/^data:(.*?);base64,(.*)$/);
                    if (!m) return null;
                ...
                function normalizeAnalysisClient(parsed) {
                    ...
                    const changes = parsed.changes
                        .map((c, idx) => {
                            const bbox = toBBoxObjectClient(
                                c.bbox ?? c.box ?? c.boundingBox
                            );
                            if (!bbox) {
                                console.warn(
                                    '[client] dropping change #' +
                                        idx +
                                        ' due to invalid bbox',
                                    c
                                );
                            ...
                            const id =
                                c.id ??
                                `change-${Date.now().toString(36)}-${idx}`;
                            const label =
                                c.label ??
                                c.changeType ??
                                c.description ??
                            ...
                            if (confidence > 1)
                                confidence =
                                    confidence > 100 ? 1 : confidence / 100;
                            ...
                            const accessibilityImpact =
                                c.accessibilityImpact ??
                                c.accessibility_impact ??
                                'None';
                            const accessibilityNotes =
                                c.accessibilityNotes ??
                                c.accessibility_notes ??
                                '';
                            const mobileImpact =
                            ...
                            const wcagAA_normal_pass =
                                c.wcagAA_normal_pass ??
                                c.wcag_aa_normal_pass ??
                                false;
                            const wcagAA_large_pass =
                                c.wcagAA_large_pass ??
                                c.wcag_aa_large_pass ??
                                false;
                ...
                if (clientApiKey && isDev) {
                    ...
                    if (!base || !newer) {
                        return Promise.reject(
                            new Error('Invalid data URLs for images')
                        );
                    }
                    ...
                    console.warn(
                        '⚠️  Using client-side Gemini key — DEVELOPMENT MODE ONLY. Do not use in production.'
                    );
                    ...
                    return fetch(endpoint, {
                        ...
                        .then((respJson) => {
                            ...
                            if (!text)
                                throw new Error(
                                    'No model text candidate in response'
                                );
                            let parsed;
                            try {
                ...
                return fetch('/api/analyze', {
                            ...
                            return response
                                ...
                                .then((data) => {
                                    ...
                                    if (response.status === 429) {
                                        console.warn(
                                            '⚠️  RATE LIMIT: Too many requests. Daily limit is 20 requests/day, 5 requests/minute, 250K tokens/minute.'
                                        );
                                        throw new Error(
                                            data.message ||
                                    ...
                                    if (response.status === 503) {
                                        console.warn(
                                            '⚠️  QUOTA EXCEEDED: Daily API quota reached. You can try again tomorrow.'
                                        );
                                        throw new Error(
                                            data.error ||
                                    ...
                                    if (response.status === 413) {
                                        throw new Error(
                                            'Images are too large (max 5MB each)'
                                        );
                                    }
                                    throw new Error(
                                        data.error ||
                    ...
                    .then((data) => {
                        ...
                        const serverLooksUnnormalized =
                            ...
                            (data?.changes?.[0] &&
                                Array.isArray(data.changes[0].bbox));
